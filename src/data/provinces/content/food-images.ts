/** Imágenes de comidas (Wikimedia Commons, uso educativo). */
export const FOOD_IMAGE_URLS: Record<string, string> = {
  "Asado bonaerense":
    "https://upload.wikimedia.org/wikipedia/commons/6/65/Asado_2005.jpg",
  "Milanesa a la napolitana":
    "https://upload.wikimedia.org/wikipedia/commons/2/24/Milanesa_napolitana_argentina.jpg",
  "Dulce de leche":
    "https://upload.wikimedia.org/wikipedia/commons/5/5a/DulceDeLeche.jpg",
  "Empanadas catamarqueñas":
    "https://upload.wikimedia.org/wikipedia/commons/3/32/Empanadas_salte%C3%B1as.jpg",
  "Locro del noroeste":
    "https://upload.wikimedia.org/wikipedia/commons/d/d1/Locro_de_Argentina.jpg",
  Tamales:
    "https://upload.wikimedia.org/wikipedia/commons/6/6b/Tamales_mexicanos.jpg",
  Chipá:
    "https://upload.wikimedia.org/wikipedia/commons/9/9e/Chipa_Paraguay.jpg",
  "Pacú a la parrilla":
    "https://upload.wikimedia.org/wikipedia/commons/3/33/Piaractus_mesopotamicus_-_Flickr_-_Dick_Culbert.jpg",
  "Mandioca hervida":
    "https://upload.wikimedia.org/wikipedia/commons/f/f1/Manihot_esculenta_-_K%C3%B6hler%E2%80%93s_Medizinal-Pflanzen-090.jpg",
  "Cordero patagónico":
    "https://upload.wikimedia.org/wikipedia/commons/c/c5/Brebis_agneau.jpg",
  "Torta galesa":
    "https://upload.wikimedia.org/wikipedia/commons/4/4c/Welsh_cakes.jpg",
  "Trucha de río":
    "https://upload.wikimedia.org/wikipedia/commons/3/3a/Oncorhynchus_mykiss.jpg",
  "Cabrito serrano":
    "https://upload.wikimedia.org/wikipedia/commons/c/cd/El_Para%C3%ADso_de_los_Animales_%2C_Cabrito_Honorio_por_Pablo_Blitto_Torre.JPG",
  "Choripán cordobés":
    "https://upload.wikimedia.org/wikipedia/commons/3/3a/Chorip%C3%A1n_argentino_con_papas_fritas.jpg",
  "Alfajor cordobés":
    "https://upload.wikimedia.org/wikipedia/commons/f/fa/Alfajores_de_maicena.jpg",
  Mbaipy:
    "https://upload.wikimedia.org/wikipedia/commons/f/f4/Humitas_por_Claudio.N%C3%BA%C3%B1ez.jpg",
  "Sopa correntina":
    "https://upload.wikimedia.org/wikipedia/commons/5/58/Sopa_Paraguaya_2.jpg",
  "Dorado a la parrilla":
    "https://upload.wikimedia.org/wikipedia/commons/4/44/Salminus_brasiliensis.jpg",
  "Pescados del Paraná y el Uruguay":
    "https://upload.wikimedia.org/wikipedia/commons/3/39/Spotted_sorubim_%E2%80%9CPseudoplatystoma_corruscans%E2%80%9D_at_MAV-USP.jpg",
  "Empanadas de pescado":
    "https://upload.wikimedia.org/wikipedia/commons/2/2e/Empanada%2C_flaky_pastry_perfection.jpg",
  "Asado con cuero":
    "https://upload.wikimedia.org/wikipedia/commons/6/65/Asado_2005.jpg",
  "Sopa paraguaya":
    "https://upload.wikimedia.org/wikipedia/commons/5/58/Sopa_Paraguaya_2.jpg",
  "Pacú al horno":
    "https://upload.wikimedia.org/wikipedia/commons/3/33/Piaractus_mesopotamicus_-_Flickr_-_Dick_Culbert.jpg",
  "Chipá cuerito":
    "https://upload.wikimedia.org/wikipedia/commons/9/9e/Chipa_Paraguay.jpg",
  "Humita en chala":
    "https://upload.wikimedia.org/wikipedia/commons/f/f4/Humitas_por_Claudio.N%C3%BA%C3%B1ez.jpg",
  "Tamales jujeños":
    "https://upload.wikimedia.org/wikipedia/commons/6/6b/Tamales_mexicanos.jpg",
  Calapurca:
    "https://upload.wikimedia.org/wikipedia/commons/a/af/Kalaphurka.jpg",
  "Asado criollo pampeano":
    "https://upload.wikimedia.org/wikipedia/commons/6/65/Asado_2005.jpg",
  "Chivito al asador":
    "https://upload.wikimedia.org/wikipedia/commons/c/cd/El_Para%C3%ADso_de_los_Animales_%2C_Cabrito_Honorio_por_Pablo_Blitto_Torre.JPG",
  "Torta frita":
    "https://upload.wikimedia.org/wikipedia/commons/7/7f/TortaFrita.jpg",
  "Chanfaina riojana":
    "https://upload.wikimedia.org/wikipedia/commons/c/c2/Navas_del_Madro%C3%B1o_Plato_de_chanfaina.jpg",
  "Empanadas riojanas":
    "https://upload.wikimedia.org/wikipedia/commons/2/2e/Empanada%2C_flaky_pastry_perfection.jpg",
  "Locro riojano":
    "https://upload.wikimedia.org/wikipedia/commons/d/d1/Locro_de_Argentina.jpg",
  "Tortitas mendocinas":
    "https://upload.wikimedia.org/wikipedia/commons/7/7f/TortaFrita.jpg",
  "Chivo a la llama":
    "https://upload.wikimedia.org/wikipedia/commons/c/cd/El_Para%C3%ADso_de_los_Animales_%2C_Cabrito_Honorio_por_Pablo_Blitto_Torre.JPG",
  "Empanadas mendocinas":
    "https://upload.wikimedia.org/wikipedia/commons/2/2e/Empanada%2C_flaky_pastry_perfection.jpg",
  Reviro:
    "https://upload.wikimedia.org/wikipedia/commons/7/7f/TortaFrita.jpg",
  "Chipá misionero":
    "https://upload.wikimedia.org/wikipedia/commons/9/9e/Chipa_Paraguay.jpg",
  Mbejú:
    "https://upload.wikimedia.org/wikipedia/commons/9/9e/Chipa_Paraguay.jpg",
  "Chivito neuquino":
    "https://upload.wikimedia.org/wikipedia/commons/c/cd/El_Para%C3%ADso_de_los_Animales_%2C_Cabrito_Honorio_por_Pablo_Blitto_Torre.JPG",
  "Trucha patagónica":
    "https://upload.wikimedia.org/wikipedia/commons/3/3a/Oncorhynchus_mykiss.jpg",
  "Torta frita patagónica":
    "https://upload.wikimedia.org/wikipedia/commons/7/7f/TortaFrita.jpg",
  "Cordero al asador":
    "https://upload.wikimedia.org/wikipedia/commons/c/c5/Brebis_agneau.jpg",
  "Frutas del valle":
    "https://upload.wikimedia.org/wikipedia/commons/1/15/Red_Apple.jpg",
  "Empanadas salteñas":
    "https://upload.wikimedia.org/wikipedia/commons/3/32/Empanadas_salte%C3%B1as.jpg",
  "Tamales salteños":
    "https://upload.wikimedia.org/wikipedia/commons/6/6b/Tamales_mexicanos.jpg",
  Humita:
    "https://upload.wikimedia.org/wikipedia/commons/f/f4/Humitas_por_Claudio.N%C3%BA%C3%B1ez.jpg",
  "Punta de espalda":
    "https://upload.wikimedia.org/wikipedia/commons/6/65/Asado_2005.jpg",
  Tomaticán:
    "https://upload.wikimedia.org/wikipedia/commons/d/d3/Tomatic%C3%A1n.jpeg",
  Sopaipillas:
    "https://upload.wikimedia.org/wikipedia/commons/e/e1/Sopaipilla.jpg",
  "Chivito puntano":
    "https://upload.wikimedia.org/wikipedia/commons/c/cd/El_Para%C3%ADso_de_los_Animales_%2C_Cabrito_Honorio_por_Pablo_Blitto_Torre.JPG",
  "Empanadas puntanas":
    "https://upload.wikimedia.org/wikipedia/commons/2/2e/Empanada%2C_flaky_pastry_perfection.jpg",
  Carbonada:
    "https://upload.wikimedia.org/wikipedia/commons/1/13/Carbonada.JPG",
  "Cazuela de mariscos":
    "https://upload.wikimedia.org/wikipedia/commons/a/a3/Seafood_stew.jpg",
  "Cordero santacruceño":
    "https://upload.wikimedia.org/wikipedia/commons/c/c5/Brebis_agneau.jpg",
  "Centolla patagónica":
    "https://upload.wikimedia.org/wikipedia/commons/5/5e/Chile_King_Crab.jpg",
  "Surubí al horno":
    "https://upload.wikimedia.org/wikipedia/commons/3/39/Spotted_sorubim_%E2%80%9CPseudoplatystoma_corruscans%E2%80%9D_at_MAV-USP.jpg",
  "Empanada santafesina":
    "https://upload.wikimedia.org/wikipedia/commons/2/2e/Empanada%2C_flaky_pastry_perfection.jpg",
  "Locro santiagueño":
    "https://upload.wikimedia.org/wikipedia/commons/d/d1/Locro_de_Argentina.jpg",
  "Empanadas santiagueñas":
    "https://upload.wikimedia.org/wikipedia/commons/2/2e/Empanada%2C_flaky_pastry_perfection.jpg",
  "Rosquete santiagueño":
    "https://upload.wikimedia.org/wikipedia/commons/4/4c/Welsh_cakes.jpg",
  "Centolla fueguina":
    "https://upload.wikimedia.org/wikipedia/commons/5/5e/Chile_King_Crab.jpg",
  "Merluza negra":
    "https://upload.wikimedia.org/wikipedia/commons/a/a8/Toothfish.jpg",
  "Cordero fueguino":
    "https://upload.wikimedia.org/wikipedia/commons/c/c5/Brebis_agneau.jpg",
  "Sánguche de milanesa":
    "https://upload.wikimedia.org/wikipedia/commons/2/24/Milanesa_napolitana_argentina.jpg",
  "Empanadas tucumanas":
    "https://upload.wikimedia.org/wikipedia/commons/3/32/Empanadas_salte%C3%B1as.jpg",
  "Humita tucumana":
    "https://upload.wikimedia.org/wikipedia/commons/f/f4/Humitas_por_Claudio.N%C3%BA%C3%B1ez.jpg",
  "Pizza porteña":
    "https://upload.wikimedia.org/wikipedia/commons/0/09/T%C3%ADpica_pizza_de_mozzarella_de_Buenos_Aires_%28Argentina%29.png",
  "Fugazzeta rellena":
    "https://upload.wikimedia.org/wikipedia/commons/8/87/Fugazzetta_en_Pizzer%C3%ADa_Guerrin%2C_Buenos_Aires.jpg",
  "Helado artesanal":
    "https://upload.wikimedia.org/wikipedia/commons/3/31/Ice_Cream_dessert_02.jpg",
  "Pescado frito con papas":
    "https://upload.wikimedia.org/wikipedia/commons/f/ff/Fish_and_chips_blackpool.jpg",
  "Merluza y mariscos locales":
    "https://upload.wikimedia.org/wikipedia/commons/a/a8/Toothfish.jpg",
  "Cordero isleño":
    "https://upload.wikimedia.org/wikipedia/commons/c/c5/Brebis_agneau.jpg",
};
